...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function X = SPC(T,Q,opts)
    TV_QV = opts.TVQV;        % 'tv' or 'qv' ;
    rho = opts.rho;          % smoothness (0.1 - 1.0) for 'qv' and (0.01 - 0.5) for 'tv' is recommended.
    K = opts.K;              % Number of components which are updated in one iteration. (typically 10)
    SNR = opts.SNR;          % error bound
    rate = opts.nu;            % threshold for R <-- R + 1.
    maxiter = opts.maxiter;  % maximum number of iteration
    tol = opts.tol;          % tolerance
    out = opts.out_im;    % you can monitor the process of 'image' completion if out == 1. 'saved' directory is necessary to save the individual rank images.
