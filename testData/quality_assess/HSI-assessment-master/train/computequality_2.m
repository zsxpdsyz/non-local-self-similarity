function  quality = computequality_2(im,mu_prisparam,cov_prisparam,principleVectors,meanOfSampleData,la_data)
...
%im = imresize(im, [normalizedWidth normalizedWidth]);    % modified by Yang
% ---------------------------------------------------------------
...
%     feat_scale = feat_scale';
     composite.data=compositeMat;
     feat_scale = computefeature(composite);
...
[row, col, band] = size(la_data);    % spectral feature, modified by Yang
block_rownum = floor(row/blocksizerow);
block_colnum = floor(col/blocksizecol);
spe_featnum = 3;

for num=1:size(la_data,3)
    %la_data(:,:,num)=255*la_data(:,:,num);
...
la_data_str.data=la_data_crop;
feat_spe_scale = spectralfeature(la_data_str);


gen_feat = [feat,feat_spe_scale];   % spectral feature, modified by Yang
feat=gen_feat;
feat(:,237:276)=[];feat(:,3:42)=[];   % remove color feature, modified by Yang
