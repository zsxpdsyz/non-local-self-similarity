...
[row, col, band] = size(la_data);    % spectral feature
block_rownum = floor(row/blocksizerow);
block_colnum = floor(col/blocksizecol);
spe_featnum = 3;
...
feat_spe_scale = blockproc(double(la_data_crop),[blocksizerow blocksizecol],@spectralfeature,...    % spectral feature
...
gen_feat = [feat,feat_spe_scale];   % spectral feature
feat=gen_feat;
feat(:,237:276)=[]; feat(:,3:42)=[];   % remove color feature
